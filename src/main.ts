...
const calcultateRevenue = (rezervations: ParsedData[], targetMonth: string) => {
  ...
  rezervations.forEach((rezervation) => {
    const startDateMonth = new Date(
      `${rezervation.startDate.getFullYear()}-${
        rezervation.startDate.getMonth() + 1
      }-01`,
    )
     ...
     **/
    if (
      targetMonthStart >= startDateMonth &&
      targetMonthStart <= rezervation.endDate
    ) {
...
const { totalRevenue, totalUnreservedCapacity } = calcultateRevenue(
  parsedData,
  monthToCheck,
)
